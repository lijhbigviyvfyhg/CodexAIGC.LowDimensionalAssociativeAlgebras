...
> **Status: incremental formalisation and literature audit.** Dimensions zero through two are complete over all three target fields. Dimensions three and four remain open, so this repository does not yet claim the requested dimension-at-most-four classification.
...
本项目目标是用 Lean 4 对不超过四维的有限维结合代数进行可核验分类。目前零至二维已经完成，三维与四维仍在进行，因而**尚未声称得到不超过四维的完整分类**。每个已完成切片都分别证明：结合律、确实定义一个同构类、候选之间两两不同构、以及对全部目标代数的存在唯一覆盖。
...
当前已在任意域上完整证明维数 0 与维数 1 的分类，因此三条目标域轨道均已覆盖这两个维数。零维只有一个同构类；一维恰有零乘法与 `e * e = e` 两个同构类。二维也已在三条轨道上完成：`ℂ` 上恰有 7 类，`ℝ` 上恰有 8 类；`𝔽₂` 上 256 张双线性乘法表中 28 张满足结合律，组成恰好 8 个 `GL(2,2)` 同构轨道。全部三、四维情形仍未完成，仓库不作整体完整性声明。
